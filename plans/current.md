...
## Next concrete step
Deploy the branch to Vercel, confirm `RESEND_API_KEY`, `CONTACT_FROM_EMAIL`, and `CONTACT_TO_EMAIL` are present, then verify four things on production: `/contact` no longer 404s on direct load, the Google Fonts CSP warning is gone, `/api/contact` no longer throws `FUNCTION_INVOCATION_FAILED`, and Gmail Reply targets the submitted visitor email before running the remaining homepage pricing preview browser QA.
